...
for selec in tqdm(range(n_items)):
    with open('features_per_pixel_cluster_wheel_bike/features_cluster_wheel_bike_part_'+str(selec)+'.pkl', 'rb') as f:
        a = pickle.load(f)
...
# 
with open('kmeans_models/kmeans_wheel_bike.pkl', 'wb') as f:
    pickle.dump(model, f)
    
with open('kmeans_models/scaler_wheel_bike.pkl', 'wb') as f:
