...
color_target = np.array([220,20,60]) #pedestrian
id_target = np.where(np.equal(color_map, color_target).all(axis=1))[0][0]

color_bike = np.array([119, 11, 32]) 
id_bike = np.where(np.equal(color_map, color_bike).all(axis=1))[0][0]



color_rider = np.array([255,0,0])
...
for selec in tqdm(range(n_images)):    
    ...
    # =============================================================================
    cimg = imageio.imread('reduced_cluster_v12/reduced_'+str(selec)+'.png')
    final_img = cimg.copy()
...
# =============================================================================
        ...
        # if d_count[id_bike] >= 0.10 * len(neigh_labels):
        if d_count[id_bike] >= 5: # this avoid single pixels to change whole pedestrian
            key_colors.append(color_rider)  
        else:
    ...
    im.save('PREDICTION_custom_v2/'+str(selec)+'.png')
