...
color_map = np.load('color_map_custom.npy')



#rellenamos primero las manchas de road
color_target = np.array([128,64,128]) #road
id_target = np.where(np.equal(color_map, color_target).all(axis=1))[0][0]

color_car = np.array([0, 0, 142]) #car
id_car = np.where(np.equal(color_map, color_car).all(axis=1))[0][0]

color_shadow = np.array([0,0,0]) #shadows
id_shadow = np.where(np.equal(color_map, color_shadow).all(axis=1))[0][0]

color_vegetation = np.array([107,142,35]) #vertical vegetation
id_vegetation = np.where(np.equal(color_map, color_vegetation).all(axis=1))[0][0]
...
for selec in tqdm(range(n_images)):    
    ...
    # =============================================================================
    cimg = imageio.imread('PREDICTION_custom/'+str(selec)+'.png')
    final_img = cimg.copy()
...
# =============================================================================
        ...
        if d_count[id_car]+d_count[id_shadow]+d_count[id_vegetation] >= 0.90 * len(neigh_labels):
    ...
    im.save('reduced_cluster_v10/reduced_'+str(selec)+'.png')
