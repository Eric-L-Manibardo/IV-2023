...
color_target = np.array([100,0,0]) #rojo wheel
id_target = np.where(np.equal(color_map, color_target).all(axis=1))[0][0]

color_bike = np.array([255,255,63]) #amarillo bike
...
for selec in tqdm(range(n_images)):    
    ...
    # =============================================================================
    cimg = imageio.imread('reduced_cluster_v7/reduced_'+str(selec)+'.png')
    final_img = cimg.copy()
...
# =============================================================================
        ...
        if d_count[id_bike] >= 1:
            key_colors.append(color_bike)
        else:
    ...
    im.save('reduced_cluster_v8/reduced_'+str(selec)+'.png')
