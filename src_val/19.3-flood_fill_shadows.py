...
color_target = np.array([0,0,0]) #shadows
...
color_road = np.array([128, 64, 128]) #road
id_road = np.where(np.equal(color_map, color_road).all(axis=1))[0][0]

color_noise = np.array([255,255,255])
...
# =============================================================================
        ...
        if d_count[id_car] >= 0.20 * len(neigh_labels):
            key_colors.append(color_car)
        elif d_count[id_road] >= 0.40 * len(neigh_labels):
    ...
    im.save('reduced_cluster_v12/reduced_'+str(selec)+'.png')
    #guardamos la version sin RIDER
    im.save('PREDICTION_custom_v2_noRIDER/'+str(selec)+'.png')
