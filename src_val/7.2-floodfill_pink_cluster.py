...
color_map = np.load('color_map_reduced_1.npy')

color_wheel_bike = np.array([233,128,233]) #pink wheel/bike
id_wheel_bike = np.where(np.equal(color_map, color_wheel_bike).all(axis=1))[0][0]

color_target = np.array([255,128,0]) #orange hvegetation
...
color_vehicle = np.array([128,64,128]) 
id_vehicle = np.where(np.equal(color_map, color_vehicle).all(axis=1))[0][0]



# color_vvegetation = np.array([102,102,156])
# id_vvegetation = np.where(np.equal(color_map, color_vvegetation).all(axis=1))[0][0]

# color_sidewalk = np.array([0,0,90])
# id_sidewalk = np.where(np.equal(color_map, color_sidewalk).all(axis=1))[0][0]

# color_railway = np.array([150,120,90]) #it also contains some sidewalk borders
# id_railway = np.where(np.equal(color_map, color_railway).all(axis=1))[0][0]
...
for selec in tqdm(range(n_images)):    
    ...
    # =============================================================================
    cimg = imageio.imread('reduced_cluster_v3/reduced_'+str(selec)+'.png')
    final_img = cimg.copy()
...
# =============================================================================
        ...
        # ruedas mal clasificadas
        if d_count[id_shadow] + d_count[id_vehicle] >= 0.50 * len(neigh_labels):
            key_colors.append(color_wheel_bike)
        #pequeñas islas de ruido
        elif d_count[id_wheel_bike] >= 0.10 * len(neigh_labels):
            key_colors.append(color_wheel_bike)
        else:
...
# =============================================================================
    ...
    im.save('reduced_cluster_v4/reduced_'+str(selec)+'.png')
