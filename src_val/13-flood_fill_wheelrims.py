...
color_target = np.array([255,255,63]) #yellow bike/wheelrim
id_target = np.where(np.equal(color_map, color_target).all(axis=1))[0][0]

color_wheel = np.array([100,0,0]) # red wheel
...
# =============================================================================
        ...
        if d_count[id_wheel]+d_count[id_car]+d_count[id_shadow] >= 0.75 * len(neigh_labels):
            key_colors.append(color_wheel)
        else:
